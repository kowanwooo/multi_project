...
public class MissingBoardServiceImpl implements MissingBoardServiceInterface {
	...
	public List<String> fileProcess(MultipartHttpServletRequest multipartRequest,
									String CURR_IMAGE_REPO_PATH) throws Exception {
		List<String> fileList = new ArrayList<String>();
		Iterator<String> fileNames = multipartRequest.getFileNames();
		while (fileNames.hasNext()) {
			String fileName = fileNames.next();
			MultipartFile mFile = multipartRequest.getFile(fileName);
			String originalFileName = mFile.getOriginalFilename(); //파일이름 추출
			fileList.add(originalFileName); //파일이름 하나씩 추가
			File file = new File(CURR_IMAGE_REPO_PATH + "/" + fileName); //파일경로 + 이름
			if (mFile.getSize() != 0) {
				...
				mFile.transferTo(new File(CURR_IMAGE_REPO_PATH + "/" + originalFileName)); //실제 업로드 처리
			}
		}
		return fileList; //추가된 파일이름 리턴
	}
